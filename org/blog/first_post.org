...
posts of hikes I did. For this, I initially had in mine a simple org
...
This is exactely what I achieved, using the javascript library Leaflet
...
Lets break this down into more understandable pieces. First, the
exterior div is used to encapsulate everything nicely. Then, the GPX
data is put inside a <script> tag, with the "text/xml" type, as GPX
belongs to the XML family. The %s will be useful later, as we're going
to use the elisp format function to plug the GPX data inside. 
...
set, either using a style attribute, or putting a snippet like this in
your CSS files : 
...
the L.map constructor. It takes either the id of the map div, or the
corresponding element object. I wanted to keep the possibility of
having several maps in a single page so a fixed id could not work, and
I did not want to bother using gensym everywhere. The map element is
thus found by navigating in the DOM. 
...
in org-mode. To do this, lets understand correctly what does this line
exactely means: 
#+BEGIN_SRC org
  ,#+INCLUDE: "parcours.gpx" export gpx
#+END_SRC
...
If you want to see a practical use of this, please visit the [[https://github.com/Joh11/website][git repo]]
for my website. It is still a heavy WIP but it should be enough to get
started. 
